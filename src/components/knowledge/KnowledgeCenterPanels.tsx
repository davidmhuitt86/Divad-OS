...
const TYPE_ICON: Record<string, string> = {
  document: 'DOC', task: 'TASK', knowledge_object: 'KO', decision: 'DDR',
  architecture_phase: 'AP', requirement: 'REQ', risk: 'RISK', research: 'R&D',
  standard: 'STD', apo: 'APO', apt: 'APT', meeting: 'MTG', journal: 'JRN',
}
        ...
        <div style={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
          {recent.length === 0 ? (
            ...
            <div key={obj.id} onClick={() => openObject(obj)} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '8px 12px', borderBottom: '1px solid #1a1e2833', cursor: 'pointer' }}
              ...
              <div style={{ width: 36, height: 28, borderRadius: 6, background: '#1a1e28', border: '1px solid #222736', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 800, color: '#94a3b8', flexShrink: 0 }}>
                {TYPE_ICON[obj.type] ?? 'OBJ'}
              </div>
        ...
        <div style={{ padding: '6px 0', overflowY: 'auto' }}>
          {featured.length === 0 ? (
            ...
            <div key={g.type} onClick={() => navigateToObjects(g.type)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 12px', cursor: 'pointer' }}
              onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = '#1a1e28'}
              onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = 'transparent'}>
              <div style={{ width: 36, height: 28, borderRadius: 6, background: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 800, color: '#94a3b8', flexShrink: 0 }}>
                {TYPE_ICON[g.type] ?? 'OBJ'}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
