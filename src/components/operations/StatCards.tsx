...
export default function StatCards({ appState, objects }: Props) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, flexShrink: 0 }}>
      ...
      >
        ...
        {totalObjects > 0 && (
          <div style={{ fontSize: 11, color: '#475569', marginTop: 6 }}>
            {approved} of {totalObjects} objects approved
          </div>
        )}
      </StatCard>
      ...
      <StatCard
        label="ARCHITECTURE PROGRESS"
        icon={<TrendingUp size={14} />}
        iconColor="#3b82f6"
        isEmpty={!appState.currentAP}
        emptyLabel="No active AP"
      >
        {appState.currentAP && (
          <>
            ...
            <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 4 }}>
              {appState.currentAP.description ?? 'Foundation Architecture'}
            </div>
            ...
            <div style={{ fontSize: 10, color: '#475569', marginTop: 3 }}>In Progress</div>
          </>
        )}
      </StatCard>
      ...
      <StatCard
        ...
        icon={<Target size={14} />}
        iconColor="#a855f7"
        isEmpty={!appState.currentAPO}
        emptyLabel="No active objective"
      >
        {appState.currentAPO && (
          <>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#e2e8f0', lineHeight: 1.3 }}>
              {appState.currentAPO.title}
            </div>
...
function StatCard({ label, icon, iconColor, isEmpty, emptyLabel, children }: {
  label: string; icon: React.ReactNode; iconColor: string
  isEmpty: boolean; emptyLabel: string; children?: React.ReactNode
}) {
  return (
    <div style={{ background: '#13161e', border: '1px solid #1a1e28', borderRadius: 8, padding: '14px 16px' }}>
