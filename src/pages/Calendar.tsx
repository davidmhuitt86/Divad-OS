...
const PRIORITY_COLOR: Record<string, string> = { critical: '#ef4444', high: '#f59e0b', medium: '#3b82f6', low: '#22c55e' }
const STATUS_COLOR:   Record<string, string> = { draft: '#3b82f6', in_review: '#f59e0b', approved: '#22c55e', published: '#22c55e', revised: '#a855f7', archived: '#475569' }
...
const PANELS = ['mainCal', 'sidebar', 'statBar']

export default function Calendar() {
  const { objects, openWizard, setActivePage, openObject } = useStore()
  const [view,    setView]    = useState('Month')
  const [curDate, setCurDate] = useState(new Date())
  const [showFilter, setShowFilter] = useState(false)
  const layout = usePageLayout('calendar', PANELS)
  ...
  const calDays = useMemo(() => {
    ...
    for (let i = firstDay - 1; i >= 0; i--)     cells.push({ date: new Date(year, month - 1, daysInPrevMonth - i), inMonth: false })
    ...
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden', background: '#0d0f14', position: 'relative' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '12px 16px 10px', borderBottom: '1px solid #1a1e28', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          ...
          <button onClick={() => setCurDate(new Date(year, month, 1))}
            style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '6px 10px', background: 'none', border: '1px solid #1a1e28', borderRadius: 5, cursor: 'pointer', fontSize: 13, fontWeight: 700, color: '#e2e8f0' }}>
            {MONTHS[month]} {year} <ChevronDown size={13} />
          </button>
          <div style={{ position: 'relative' }}>
            <button onClick={() => setShowFilter(f => !f)}
              style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 12px', background: showFilter ? 'rgba(59,130,246,0.1)' : '#1a1e28', border: `1px solid ${showFilter ? 'rgba(59,130,246,0.3)' : '#222736'}`, borderRadius: 5, cursor: 'pointer', fontSize: 11, color: showFilter ? '#3b82f6' : '#94a3b8' }}>
              <Filter size={11} /> Filters
            </button>
            {showFilter && (
              <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: 4, background: '#13161e', border: '1px solid #222736', borderRadius: 8, padding: 12, zIndex: 50, minWidth: 180, boxShadow: '0 8px 24px rgba(0,0,0,0.4)' }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>Filter Events</div>
                {['Tasks', 'Meetings', 'Milestones', 'Deadlines'].map(f => (
                  <label key={f} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0', cursor: 'pointer' }}>
          ...
          {view === 'Month' ? (
            <>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', borderBottom: '1px solid #1a1e28', flexShrink: 0 }}>
                {DOW.map(d => <div key={d} style={{ padding: '6px 10px', textAlign: 'center', fontSize: 9, fontWeight: 700, color: '#475569', letterSpacing: '0.08em' }}>{d}</div>)}
              </div>
              <div style={{ flex: 1, display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gridTemplateRows: 'repeat(6, 1fr)', overflow: 'hidden' }}>
                {calDays.map((cell, i) => {
                  ...
                  return (
                    <div key={i}
                      onClick={() => openWizard('meeting')}
                      style={{ border: '1px solid #1a1e2866', padding: '4px 6px', background: today_ ? 'rgba(59,130,246,0.05)' : 'transparent', cursor: 'pointer', overflow: 'hidden', minHeight: 0 }}
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 22, height: 22, borderRadius: '50%', background: today_ ? '#3b82f6' : 'transparent', marginBottom: 2 }}>
                        <span style={{ fontSize: 11, fontWeight: today_ ? 700 : 400, color: today_ ? '#fff' : inMonth ? '#94a3b8' : '#2a3042' }}>{cell.date.getDate()}</span>
                      </div>
                      {dayMeetings.slice(0, 1).map(m => (
                        <div key={m.id} onClick={e => { e.stopPropagation(); openObject(m) }}
                          style={{ fontSize: 9, color: '#e2e8f0', background: '#3b82f622', border: '1px solid #3b82f644', borderRadius: 3, padding: '1px 4px', marginBottom: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', cursor: 'pointer' }}>
                          🤝 {m.title}
                        </div>
                      ))}
                      {dayTasks.slice(0, 1).map(t => (
                        <div key={t.id} onClick={e => { e.stopPropagation(); openObject(t) }}
                          style={{ fontSize: 9, color: '#22c55e', background: '#22c55e22', border: '1px solid #22c55e44', borderRadius: 3, padding: '1px 4px', marginBottom: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', cursor: 'pointer' }}>
                          ✅ {t.title}
                        </div>
                      ))}
                    </div>
            ...
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#2a3042', fontStyle: 'italic', fontSize: 13, flexDirection: 'column', gap: 8 }}>
              <div>{view} view</div>
              <button onClick={() => setView('Month')} style={{ fontSize: 11, color: '#3b82f6', background: 'none', border: '1px solid rgba(59,130,246,0.3)', borderRadius: 5, padding: '5px 12px', cursor: 'pointer' }}>← Back to Month</button>
            </div>
          )}
        </div>
        ...
        <div style={{ borderTop: '1px solid #1a1e28', display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', flexShrink: 0 }}>
          {[
            { icon: '📅', label: 'Events This Month', value: meetings.length > 0 ? meetings.length : '—', action: () => setView('Agenda') },
            { icon: '✅', label: 'Tasks Due',          value: tasks.length > 0 ? tasks.length : '—',      action: () => setActivePage('workspace') },
            { icon: '🤝', label: 'Meetings',           value: meetings.length > 0 ? meetings.length : '—', action: () => setView('Agenda') },
            { icon: '👥', label: 'Attendees',          value: '—', action: () => {} },
            { icon: '🎯', label: 'Focus Time',         value: '—', action: () => setActivePage('workspace') },
            { icon: '🚩', label: 'Deadlines',          value: '—', action: () => setActivePage('workspace') },
          ].map((s, i) => (
            <div key={i} onClick={s.action}
              style={{ padding: '10px 12px', borderRight: i < 5 ? '1px solid #1a1e28' : 'none', display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}
      ...
      <div style={{ width: 260, borderLeft: '1px solid #1a1e28', display: 'flex', flexDirection: 'column', overflowY: 'auto', flexShrink: 0 }}>
        ...
        <div style={{ borderBottom: '1px solid #1a1e28', padding: '12px 14px' }}>
          ...
          {meetings.length === 0 ? (
            ...
            <div key={m.id} onClick={() => openObject(m)} style={{ display: 'flex', gap: 8, marginBottom: 8, cursor: 'pointer', padding: '4px', borderRadius: 4 }}
        ...
        <div style={{ borderBottom: '1px solid #1a1e28', padding: '12px 14px' }}>
          ...
          {tasks.length === 0 ? (
            ...
            <div key={t.id} onClick={() => openObject(t)}
              style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 7, cursor: 'pointer', padding: '3px', borderRadius: 4 }}
              onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = '#1a1e28'}
              onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = 'transparent'}>
              ...
              {t.priority && <span style={{ fontSize: 9, fontWeight: 600, color: PRIORITY_COLOR[t.priority], flexShrink: 0 }}>{t.priority.charAt(0).toUpperCase() + t.priority.slice(1)}</span>}
            </div>
          ))}
        </div>
        ...
        <div style={{ borderBottom: '1px solid #1a1e28', padding: '12px 14px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', marginBottom: 4 }}>
            {['Su','Mo','Tu','We','Th','Fr','Sa'].map(d => <div key={d} style={{ textAlign: 'center', fontSize: 8, color: '#475569', fontWeight: 700, padding: '2px 0' }}>{d}</div>)}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 1 }}>
            {miniCalDays.map((cell, i) => {
              const tod = isToday(cell.date)
              return (
                <div key={i} onClick={() => { setCurDate(new Date(cell.date)); setView('Month') }} style={{ textAlign: 'center', padding: '2px', cursor: 'pointer' }}>
        ...
        <div style={{ padding: '12px 14px' }}>
          ...
          {meetings.length === 0 ? (
            ...
            <div key={m.id} onClick={() => openObject(m)} style={{ display: 'flex', gap: 8, marginBottom: 10, cursor: 'pointer' }}
              onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = '#1a1e28'}
              onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = 'transparent'}>
              <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#3b82f6', marginTop: 4, flexShrink: 0 }} />
              <div>
